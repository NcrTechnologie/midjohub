...
trait FirebaseAuthTrait
{
    ...
    private function getFirebaseFactory()
    {
        try {
            return (new Factory)
                ->withServiceAccount(Storage::get(setting('serviceKeyPath', '/storage/vault/firebase_service.json')));
        } catch (\Exception $ex) {
    ...
    {
        $client = new FirestoreClient(setting('projectId', "midjo-ec780"), setting('apiKey', "AIzaSyCaB9k6IdkKgtwqVOOynOWm5kx5KOK8onQ"), [
            'database' => '(default)',
        ]);
        //set auth for client
